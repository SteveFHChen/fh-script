...
x=torch.linspace(-10,10,20).reshape(20,1)
#建立从-1到1之间一百个点并且改变他的阶数（从【1,100】变到【100,1】）
#y=x
y=x.pow(2)
#y=x**3
#建立x与y之间的关系y=x^2
y_real=torch.normal(y,5.0)
...
net=Net(1,80,1)#我假设一个输入，隐藏层里有80个神经元，和一个输出
...
#Show train graph
plt.plot(x.tolist(),prediction.tolist(),color="red", linewidth=2.0)


x_test_float = torch.tensor(np.array([-15, -13, -12, -8, -7, -4, -3.5, -2.8, -1.1, -0.3, 
          0.3, 1.2, 2.5, 3.3, 4, 7, 8, 11, 12, 14]).reshape(-1,1)).float()
